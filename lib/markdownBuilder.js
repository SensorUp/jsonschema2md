...
function build({
  ...
  function makeheader(schema) {
    console.log('making header for', schema[s.filename], schema[s.pointer]); // eslint-disable-line no-console
    if (header) {
      return [
  ...
  // if object type is unstructured, omit link
  function unStructured(definition) {
    const noDefinitionTypes = ['string', 'boolean', 'number'];
    const definitionType = definition[keyword`type`];
    return noDefinitionTypes.includes(definitionType);
  }
  ...
  function makedefinedinfact(definition) {
    const definedinlink = (() => {
      if (dense && definition[keyword`title`] && typeof definition[keyword`title`] === 'string') {
        // if the type has a title, always create a link to the schema
        return [link(`${definition[s.slug]}.md`, '', text(definition[keyword`title`]))];
      }
      return [link(`${definition[s.slug]}.md`, `${definition[s.id]}#${definition[s.pointer]}`, text(definition[s.titles] && definition[s.titles][0] ? definition[s.titles][0] : i18n`Untitled schema`))];
    })();
    if (dense && unStructured(definition)) {
      return listItem(paragraph([text(i18n`Unstructured type`)]));
    }
    return listItem(paragraph([text(i18n`defined in: `), ...definedinlink]));
  }
  ...
  console.log('generating markdown'); // eslint-disable-line no-console
