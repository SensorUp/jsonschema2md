...
function build({
  ...
  function type(property) {
    if (typeof property[keyword`type`] === 'object') {
      if (omitTypeSection) return text(i18n`Complex`);
  ...
  function makedefinedinfact(definition) {
    // if object type is string/boolean, omit link
    const definitionType = definition[keyword`type`];
    const noDefinitionTypes = ['string', 'boolean', 'number'];
    const definedinlink = (() => {
      if (omitTypeSection && definition[keyword`title`] && typeof definition[keyword`title`] === 'string') {
        // if the type has a title, always create a link to the schema
    ...
    if (omitTypeSection && noDefinitionTypes.includes(definitionType)) {
  ...
  function maketypesection(schema, level = 1) {
    if (omitTypeSection && level > 1) return [];
