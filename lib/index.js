...
async function main(args) {
  ...
  const { argv } = yargs(args)
    ...
    .alias('s', 'sparse')
    .boolean('s')
    .describe('s', 'sparse output includes redundant information in output and spreads it to linked pages')
    .default('s', false)
  ...
  const schemaloader = loader({ sparse: argv.s });
  ...
  if (argv.x !== '-') {
    console.log('writing schemas'); // eslint-disable-line no-console
    writeSchema({
      schemadir: argv.x,
      origindir: argv.d,
    })(loadedschemas);
  ...
  if (!argv.n) {
    console.log('writing README'); // eslint-disable-line no-console
    pipe(
      loadedschemas,
      // build readme
      readme({
  ...
  console.log('writing documentation'); // eslint-disable-line no-console
  pipe(
    loadedschemas,
